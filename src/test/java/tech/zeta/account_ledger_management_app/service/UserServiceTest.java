...
@ExtendWith(MockitoExtension.class)
 class UserServiceTest {
    ...
    @Test
    void testAddUser() {
        when(tenantRepository.existsByRegisteredUserIdsContaining(users.getUserId()))
                .thenReturn(true);
        when(userRepository.save(users))
                .thenReturn(users);
    ...
    void testVerifyUser_whenAuthenticated_shouldReturnToken() {
        LoginRequest loginRequest = new LoginRequest("test-user", "test123");
        Authentication authentication = Mockito.mock(Authentication.class);
        when(authenticationManager.authenticate(any(UsernamePasswordAuthenticationToken.class)))
                .thenReturn(authentication);
        when(authentication.isAuthenticated())
                .thenReturn(true);
        when(jwtService.generateToken("test"))
    ...
    void testGetUserIdDetails() {
        when(userRepository.findById(users.getUserId()))
                .thenReturn(java.util.Optional.of(users));
    ...
    void testUserUpdateRequestUserName(){
        ...
        when(userRepository.findById(users.getUserId())).thenReturn(Optional.of(users));
        UsersDTO usersDTO = userService.updateDetails(1L,request);
        assertEquals("updated-test-user",usersDTO.getUsername());
    ...
    void testDeleteUserNotFound() {
        when(userRepository.findById(users.getUserId()))
                .thenThrow(new UserNotFoundException("User Not Found, Please check the Id provided: "+ users.getUserId()));
        Long userId = users.getUserId();
